...
psm_df['USI'] = psm_df.apply(lambda row: 'mzspec:' + args.pride_accession + ':' + row['SpectrumFilename'] + ':' + row['SpectrumTitle'].split('scan=',1)[1] + ':' + replace_mods(row['SequenceWithMods']) + '/' + ('2' if row['charge_2'] else ('3' if row['charge_3'] else '4')), axis=1)
